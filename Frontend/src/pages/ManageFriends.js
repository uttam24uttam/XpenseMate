...
// The Manage Friends page is handling:
// Fetching and showing the user’s friends.
// Adding a friend (via email).
// Showing overall balance (settled / owed / owing).
// Navigating to individual friend details.
// Splitting expenses with friends (via AddFriendExpense modal).

//states
// const [friends, setFriends] = useState([]);            // list of friends for current user
// const [loading, setLoading] = useState(false);         // loading state for friends list
// const [searchEmail, setSearchEmail] = useState("");    // email entered in Add Friend modal
// const [showAddFriendModal, setShowAddFriendModal] = useState(false); // modal toggle
// const [addExpenseModalVisible, setAddExpenseModalVisible] = useState(false); // expense modal toggle
// const [overallBalanceMessage, setOverallBalanceMessage] = useState(""); // string from backend


//api call
// const response = await axios.get(
//     `http://localhost:5000/api/friends/get-friends/${user._id}`
// );
// setFriends(response.data);
// Fetches all friends for the logged-in user.
// Stores them in friends state.
// const response = await axios.get(
//   `http://localhost:5000/api/friends/get-overall-balance/${user._id}`
// );
// setOverallBalanceMessage(response.data.message);
...
//   getFriends();
// }, [dependecy]);
//so here the getFriends function will be called whenever the component mounts or when the dependency changes.
