...
// getTransaction → fetches all transactions from backend.

// deleteTransaction → deletes a specific transaction.

//  downloadPDF → generates and downloads a PDF of transaction data.

// 🔹 API Calls

// POST / api / transactions / get - all - transactions → fetch transactions based on userId, frequency, dateRange, type.

// POST / api / transactions / delete -transaction → delete a transaction by transactionId.
...
// (like fetching data, updating the DOM, setting up timers, or subscribing to events).

// axios is used for making HTTP requests (GET, POST, PUT, DELETE).

// An HTTP request is a message sent by a client (like your React app running in a browser) to a server, asking it to perform some action (like fetching data, saving data, deleting data, etc.).
// HTTP stands for Hypertext Transfer Protocol — the protocol that defines how communication happens between a client (browser/app) and a server.
// The request contains information like:
// Method (GET, POST, PUT, DELETE, etc.)
// URL (the endpoint where the request is sent)
// Headers (extra info like authentication tokens, content type, etc.)
// Body/Data (for POST/PUT requests, you send data here)


// GET → Retrieve data from the server (read-only, no body).

// POST → Send data to the server to create or process something (can also return data).
...
// OPTIONS → Ask the server which methods/operations are allowed on a resource.